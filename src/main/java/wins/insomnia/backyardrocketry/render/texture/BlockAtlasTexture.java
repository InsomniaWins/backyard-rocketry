...
public class BlockAtlasTexture extends Texture {
	...
	public BlockAtlasTexture() {
		...
		registerBlockTexture("cobblestone", new BlockTextureFrames(
				new Vector2i(0, 0))
		);
		registerBlockTexture("stone", new BlockTextureFrames(
				new Vector2i(1, 0))
		);
		registerBlockTexture("dirt", new BlockTextureFrames(
				new Vector2i(2, 0))
		);
		registerBlockTexture("grass_bottom", new BlockTextureFrames(
				new Vector2i(3, 0))
		);
		registerBlockTexture("grass_top", new BlockTextureFrames(
				new Vector2i(4, 0))
		);
		registerBlockTexture("log_top", new BlockTextureFrames(
				new Vector2i(0, 1))
		);
		registerBlockTexture("log_side", new BlockTextureFrames(
				new Vector2i(1, 1))
		);
		registerBlockTexture("leaves", new BlockTextureFrames(
				new Vector2i(2, 1))
		);
		registerBlockTexture("wooden_planks", new BlockTextureFrames(
				new Vector2i(3, 1))
		);
		registerBlockTexture("glass", new BlockTextureFrames(
				new Vector2i(4, 1))
		);
		registerBlockTexture("bricks", new BlockTextureFrames(
				new Vector2i(5, 1))
		);
		registerBlockTexture("wood", new BlockTextureFrames(
				new Vector2i(0, 2))
		);
		registerBlockTexture("water", new BlockTextureFrames(20, new Vector2i[] {
				new Vector2i(6, 0),
				new Vector2i(1, 1),
				new Vector2i(4, 1)
		}));
	...
	public static class BlockTextureFrames {
		...
		private final int[] FRAME_POSITIONS;

		public BlockTextureFrames(Vector2i framePosition) {
			this(1, new Vector2i[] {framePosition});
		}

		public BlockTextureFrames(int ticksPerFrame, Vector2i[] framePositions) {

			TICKS_PER_FRAME = ticksPerFrame;
			FRAME_AMOUNT = framePositions.length;

			FRAME_POSITIONS = new int[FRAME_AMOUNT * 2];

			for (int i = 0; i < FRAME_AMOUNT; i++) {
				FRAME_POSITIONS[i*2] = framePositions[i].x;
				FRAME_POSITIONS[i*2 + 1] = framePositions[i].y;
			}
		...
		public int[] getFramePosition(int frameIndex) {
			...
			frame[0] = FRAME_POSITIONS[frameIndex * 2];
			frame[1] = FRAME_POSITIONS[frameIndex * 2 + 1];
		...
		public double getFramesPerSecond() {
			return TICKS_PER_FRAME / (double) Updater.FIXED_UPDATES_PER_SECOND;
		}
